/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.archanpatkar.Utilities;

import com.archanpatkar.Initializer.MainThread;
import com.archanpatkar.Network.NetworkObjects;
import org.junit.After;
import org.junit.AfterClass;
import org.junit.Before;
import org.junit.BeforeClass;
import org.junit.Test;
import static org.junit.Assert.*;

/**
 *
 * @author Archan Patkar
 */
public class ThreadUtilityIT {
    
    public ThreadUtilityIT() {
    }
    
    @BeforeClass
    public static void setUpClass() {
    }
    
    @AfterClass
    public static void tearDownClass() {
    }
    
    @Before
    public void setUp() {
    }
    
    @After
    public void tearDown() {
    }

    /**
     * Test of DoConV method, of class ThreadUtility.
     */
    @Test
    public void testDoConV() {
        System.out.println("DoConV");
        NetworkObjects NO = null;
        MainThread TI = null;
        ThreadUtility.DoConV(NO, TI);
        // TODO review the generated test code and remove the default call to fail.
        fail("The test case is a prototype.");
    }
    
}
